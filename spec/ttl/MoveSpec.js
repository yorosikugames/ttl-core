...
describe('Move', function() {
    ...
    xit('액터 하나 우로 이동 (이동 불가 확인)', function() {
        var a = new Actor();
        var dix = 1,
            diy = 0;
        var action = new WorldMoveAction(dix, diy);
    ...
    /*
        [A][B][ ] -> [ ][A][B]
    */
    xit('액터 둘이 인접한 상태로 함께 오른쪽 방향으로 이동하기', function() {
    ...
    xit('같은 셀에 두 액터가 동시에 들어가려고 할 때 먼저 들어가길 요청한 액터 하나만 들어감', function() {
        var set1 = SpawnActorMoveDelta(0, 0, 1, 0);
